...
function UserSignup() {
  const {register, authState, handleUserSignup, handleSubmit} = useAuthLogic()
  return (
    <div className='flex h-full flex-row items-center justify-center bg-slate-200 w-full'>
      <form action="" className="py-4 px-8 basis-6/12 rounded-md shadow-lg bg-white" onSubmit={handleSubmit(handleUserSignup)}>
        ...
        <div className="w-full flex flex-row justify-between p-2">
          ...
          <input type="submit" className="basis-5/12 py-2 text-uppercase bg-sky-600 font-font text-white" value={authState.pending?"Loading...":"Signup"} />
        </div>
      </form>
    </div>
  )
